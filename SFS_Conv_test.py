...
class SFS_Conv(nn.Module):
    ...
    def forward(self, x):
        x_PWC0 = self.PWC0(x)
        visualize_feature_maps(x_PWC0, 'Pointwise convolution (PWC0)')
        x_spa = self.SPU(x_PWC0)
        x_PWC1 = self.PWC1(x_spa)
        visualize_feature_maps(x_spa, 'Spatial perception unit (SPU)')
        x_fre = self.FPU(x_PWC1)
        visualize_feature_maps(x_fre, 'Pointwise convolution (PWC1)')
        out = torch.cat([x_spa, x_fre], dim=1)
        visualize_feature_maps(out, 'Concatenation (torch.cat)')
        out = F.softmax(self.advavg(out), dim=1) * out
        visualize_feature_maps(out, 'Concatenation (torch.cat)')
        out1, out2 = torch.split(out, out.size(1) // 2, dim=1)
        result = self.PWC_o(out1 + out2)
        visualize_feature_maps(result, 'Concatenation (torch.cat)')

        return result


def visualize_feature_maps(feature_maps, title, max_cols=8):
    """
    Визуализирует все каналы feature maps в grid-раскладке
    """
    # Преобразование тензора и подготовка данных
    features = feature_maps.squeeze(0).detach().cpu().numpy()
    n_channels = features.shape[0]

    # Нормализация по всем каналам
    global_min = features.min()
    global_max = features.max()
    features = (features - global_min) / (global_max - global_min + 1e-6)

    # Рассчет структуры grid
    cols = min(n_channels, max_cols)
    rows = ceil(n_channels / cols)

    # Создание фигуры
    fig, axes = plt.subplots(rows, cols, figsize=(cols * 2, rows * 2))
    fig.suptitle(f"\n{title}, Channels: {n_channels}, Shape: {features.shape[1:]}", y=1.02)
...
if __name__ == "__main__":
    ...
    img = Image.open('datasets/HRSID/images/train/P0001_0_800_7200_8000.jpg').convert('RGB')

    # Обрезка и преобразование в тензор
    ...
    conv1 = Conv(channel, 32, 6, 2, 2)
    conv2 = Conv(32, 64, 3, 2)
    sfs_conv = SFS_Conv(64, 64)

    result_conv1 = conv1(img_tensor)
    result_conv2 = conv2(result_conv1)
    result_sfs_conv = sfs_conv(result_conv2)

    # flops, params = thop.profile(model, inputs=(torch.randn(1, channel, height, width),), verbose=False)
    # flops, params = thop.profile(sfs_conv, inputs=(result_conv2,), verbose=False)
    ...
    # Визуализация
    # visualize_feature_maps(result_conv1, "Conv1 Output Features")
    # visualize_feature_maps(result_conv2, "Conv2 Output Features")
    # visualize_feature_maps(result_conv2, "SFS_Conv Output Features")
